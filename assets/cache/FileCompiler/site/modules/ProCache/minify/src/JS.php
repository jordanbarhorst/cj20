...
class JS extends Minify
{
    ...
    public function __construct()
    {
        ...
        $dataDir = \ProcessWire\wire("config")->paths->root . 'site/modules/ProCache/minify/src'.'/../data/js/';
    ...
    public function execute($path = null)
    {
		...
		$noMinifyData = array(); // keys are substitution strings, values are replacement values
		$noMinifyCnt = 0; // current item count (whether minified or not)
		$noMinifyTotal = count($this->data); // total items count (whether minified or not)
		// End added by RJC/ProCache
        ...
        foreach ($this->data as $source => $js) {
			...
			// Added by RJC/ProCache -------------------------------------
			if(strpos($js, '/*NoMinify*/') === 0) {
				$noMinifyKey = '_' . md5($js) . '=1;';
				if(($noMinifyCnt+1) == $noMinifyTotal) $noMinifyKey = rtrim($noMinifyKey, ';');
				$js = str_replace('/*NoMinify*/', ($noMinifyCnt ? "\n" : ""), $js);
				$noMinifyData[$noMinifyKey] = $js;
				$js = $noMinifyKey;
			}
			$noMinifyCnt++;
			// End added by RJC/ProCache ---------------------------------
		...
		// Added by RJC/ProCache --------------------------------------------------------------------
		if(count($noMinifyData)) {
			$content = str_replace(array_keys($noMinifyData), array_values($noMinifyData), $content);
		}
		// End added by RJC/ProCache ----------------------------------------------------------------
