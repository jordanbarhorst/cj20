...
class ProCacheFileMerger extends \ProcessWire\Wire {
	...
	public function __construct(array $files = array(), $destination = '') {
		...
		if($destination) {
			...
			if(!is_dir($destination)) \ProcessWire\wiremkdir($destination);
			$this->setDestination($destination);
		}
		$src = dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/ProCache/ProCacheFileMerger.php') . '/minify/src/';
		foreach(array('Exception', 'Minify', 'Converter', 'CSS', 'JS') as $f) {
 require_once(\ProcessWire\wire('files')->compile("$src$f.php",array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
		}
	...
	public function setDestination($destination) {
		if(!is_dir($destination)) throw new \ProcessWire\WireException("Please specify a destination dir that exists"); 
		if(!is_writable($destination)) throw new \ProcessWire\WireException("Destination dir is not writable");
	...
	public function merge($files, $minify = false, $sri = false) {
		...
		if(is_file($targetFilename) && !self::debug) {
			...
			if($minifier) {
				...
				\ProcessWire\wirechmod($targetFilename);
				$this->clean();
			}
		...
		if($sri) {
			...
			\ProcessWire\wirechmod($hashFilename);
	...
	public function mergeCSSFilesInMarkup($html, $minify = true) {
		...
		$timer = $this->wire('config')->debug ? \ProcessWire\Debug::timer() : null;
		...
		foreach($filesByGroup as $group => $files) {
			...
			if($mergedLink) {
				...
				$debugTimer = $timer ? "<!--PWPC" . \ProcessWire\Debug::timer($timer) . "-->": '';
	...
	public function getHashSRI() {
		if(empty($this->hashSRI)) {
			throw new \ProcessWire\WireException('You must call merge() with the $sri option as true before calling getHashSRI()');
		}
		return $this->hashSRI;
	}
 ...
 require_once(\ProcessWire\wire('files')->compile(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/ProCache/ProCacheFileMerger.php') . '/scssphp/scss.inc.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
 ...
 require_once(\ProcessWire\wire('files')->compile(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/ProCache/ProCacheFileMerger.php') . '/lessphp/lessify.inc.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
		$less = new lessc;
